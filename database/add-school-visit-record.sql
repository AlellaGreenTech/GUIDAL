-- Add only the missing school_visits record for Benjamin Franklin
-- This assumes the activity already exists (which it does based on the duplicate error)

-- First, get the activity ID for the Benjamin Franklin visit
-- Then create the school_visits record with the correct access code
...
SELECT 'New school visit record:' as info;
SELECT
  sv.access_code,
  sv.student_count,
  sv.grade_level,
