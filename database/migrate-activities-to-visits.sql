-- Migration script to move visit records from activities table to visits table
-- This will clean up the database structure properly
...
SELECT
    CASE
        ...
        ELSE title
    END as school_name,
    max_participants as student_count,
    CASE
...
SELECT
    CASE
        ...
        ELSE title
    END as school_name,
    ROUND(COALESCE(max_participants, 0) * 0.2) as student_count, -- 20% more children than adults
    max_participants as teacher_count, -- adults
    'migrated@example.com' as contact_email,
    'Migrated from activities table' as lead_teacher_contact,
...
SELECT
    CASE
        ...
        ELSE title
    END as school_name,
    0 as student_count, -- Workshops typically don't have students
    max_participants as teacher_count, -- adults
    'migrated@example.com' as contact_email,
    'Migrated from activities table' as lead_teacher_contact,
...
-- Step 5: Verification queries (run these to check results)
...
-- SELECT SUM(student_count + teacher_count) as total_visitors FROM visits;