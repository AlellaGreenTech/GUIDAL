-- GUIDAL Complete Activities Data Migration
-- This file converts all static HTML activity data to database records
-- Run this after the complete-schema.sql file

-- =====================================================
-- SECTION 1: ADDITIONAL SCHOOLS
-- =====================================================

-- Insert additional schools not in the existing schools-data.sql
INSERT INTO public.schools (name, address, contact_person, contact_email, created_at) VALUES
('CAS TRIPS School', 'Multiple Locations', 'Tom Wolverton', 'contact@castrips.com', NOW()),
('H-Farm International', 'Italy', 'Academic Coordinator', 'contact@h-farm.com', NOW())
ON CONFLICT (name) DO NOTHING;

-- =====================================================
-- SECTION 2: ACTIVITY TYPES (already created in schema)
-- =====================================================
-- Activity types are already created in complete-schema.sql:
-- school-visits, workshops, events, lunches, camps

-- Add annual-events type
INSERT INTO public.activity_types (name, slug, description, color, icon, active)
VALUES ('Annual Events', 'annual-events', 'Yearly recurring community events', '#8bc34a', '🌟', true)
ON CONFLICT (slug) DO NOTHING;

-- =====================================================
-- SECTION 3: ALL ACTIVITIES DATA
-- =====================================================
...
-- =====================================================
-- SECTION 4: SCHOOL VISITS DETAILS
-- =====================================================

-- Insert school visit details for activities that have specific school associations
INSERT INTO public.school_visits (
  activity_id, school_id, teacher_name, teacher_email, student_count,
  grade_level, access_code, lunch_required, special_instructions
) VALUES
...
(
  ...
  'BFIS-2025-SEP',
  true,
  'IBDP Collaborative Sciences Project for 6 courses: biology, chemistry, ESS, physics, computer science, and design technology.'
),

-- International School of Prague
(
  (SELECT id FROM public.activities WHERE slug = 'international-school-prague-sept-2025'),
  ...
  'ISP-2025-SEP',
  true,
  'Focus on IoT sensors, automation, smart farming, and vineyard management.'
),

-- Zurich International School visits
(
  (SELECT id FROM public.activities WHERE slug = 'zurich-international-aug-27-2025'),
  (SELECT id FROM public.schools WHERE name = 'Zurich International School'),
  'Sustainability Coordinator',
  'sustainability@zis.ch',
  24,
  'Mixed grades',
  'ZIS-2025-AUG27',
  true,
  'Focus on sustainability and permaculture principles.'
),

(
  (SELECT id FROM public.activities WHERE slug = 'zurich-international-aug-26-2025'),
  (SELECT id FROM public.schools WHERE name = 'Zurich International School'),
  'Sustainability Coordinator',
  'sustainability@zis.ch',
  27,
  ...
  'ZIS-2025-AUG26',
  true,
  'Focus on sustainable practices and agricultural technology.'
),

-- St George
(
  (SELECT id FROM public.activities WHERE slug = 'st-george-april-2025'),
  (SELECT id FROM public.schools WHERE name = 'St George'),
  ...
  'STG-2025-APR',
  true,
  'Computer science GCSE class focusing on automated systems and AI applications.'
),

-- Learnlife
(
  (SELECT id FROM public.activities WHERE slug = 'learnlife-dec-2024'),
  (SELECT id FROM public.schools WHERE name = 'Learnlife'),
  'AP Environmental Science Teacher',
  'enviro@learnlife.edu',
  14,
  'AP Level',
  'LL-2024-DEC',
  false,
  'AP Environmental Science focus on land and water use studies.'
),

-- St. Patricks International School
(
  (SELECT id FROM public.activities WHERE slug = 'st-patricks-nov-2024'),
  (SELECT id FROM public.schools WHERE name = 'St. Patricks International School'),
  'Biology Department',
  'biology@stpatricks.edu',
  27,
  'Mixed grades',
  'SPI-2024-NOV',
  true,
  'Ecosystem and habitat exploration with hands-on permaculture activities.'
),

-- Barcelona Montessori School
(
  (SELECT id FROM public.activities WHERE slug = 'barcelona-montessori-nov-2024'),
  (SELECT id FROM public.schools WHERE name = 'Barcelona Montessori School'),
  'Montessori Educator',
  'educator@bmontessori.edu',
  35,
  'Elementary',
  'BMS-2024-NOV',
  true,
  'Hands-on learning with clay building and sustainable farming. Pizza making included!'
);

-- =====================================================
-- SECTION 5: UPDATE CREDITS REQUIREMENTS
-- =====================================================

-- Update activities that cost GREENs (workshops)
UPDATE public.activities
SET credits_required = 3, credits_earned = 0
WHERE slug = 'ram-pump-workshop-sept-2025';

-- =====================================================
-- SECTION 6: ACTIVITY COMPLETION
-- =====================================================

-- All static HTML activities are now converted to database records
-- Total: 25 unique activities
-- - 4 upcoming activities (2025)
-- - 4 completed school visits (2025)
-- - 10 completed school visits (2024 and earlier)
-- - 7 annual/special events (2021-2025)

-- Next step: Update the frontend JavaScript to use database data instead of static HTML